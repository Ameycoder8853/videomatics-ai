...
import { Eye, Edit3, Trash2, PlayCircle, AlertTriangle, Clock } from 'lucide-react';
import type { VideoDocument } from '@/firebase/firestore'; // Assuming this interface exists

// If VideoDocument is not directly usable or needs adjustment for client-side display
interface DisplayVideo extends Omit<VideoDocument, 'createdAt'> {
  createdAt: Date; // Ensure createdAt is a Date object for formatting
  thumbnailUrl: string; // Ensure this is always present for display
...
export function DashboardList({ videos }: DashboardListProps) {
  const getStatusBadgeVariant = (status: DisplayVideo['status']) => {
    switch (status) {
      ...
      case 'processing':
        return 'secondary'; // Blue or yellow
      case 'failed':
          ...
          <CardFooter className="p-4 border-t bg-muted/30">
            <div className="flex w-full justify-between items-center space-x-2">
              ...
              <div className="flex space-x-1">
                <Button variant="ghost" size="icon" className="h-8 w-8" disabled> {/* Edit disabled for now */}
                  <Edit3 className="h-4 w-4" />
                  <span className="sr-only">Edit</span>
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" disabled> {/* Delete disabled for now */}
